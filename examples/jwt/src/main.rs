...
use feather::{middleware, next, App, AppContext, Claim};
use feather::jwt::Claim;
use feather::{jwt_required};
use feather::jwt::{self,with_jwt_auth, JwtManager};
...
fn main() {
    ...
    app.get("/token1", |_req: &mut feather::Request, res: &mut feather::Response, ctx: &mut AppContext| {
    ...
    app.post("/token2", middleware!(|req,res,ctx|{
        let name = req.json()?.get("name").unwrap_or(&feather::Value::String("No Name".into())).to_string();
        let exp = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() as usize + 3600; // 1 saat sonrası
        let claims = MyClaim{
            name,
            exp,
    ...
    app.get("/protected1", with_jwt_auth::<jwt::SimpleClaims,_>(|_req,res,_ctx,claims|{
